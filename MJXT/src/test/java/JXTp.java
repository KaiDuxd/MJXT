...
public class JXTp {
    ...
    public static void main(String[] args) {
        ...
        int key_colnum = 5;//the values{5, 6, 7, 8, 9} for our dataset
        //the number join attributes in the table.
        int join_column = 5;//the values{5, 4, 3, 2, 1} for our dataset
            ...
            for (int i = 0; i < res.size(); i++) {
                if (i % 2 == 0){
                    ...
                    for (int j = 0; j < res_i.size(); j++) {
                        if (x == 0) System.out.print(new String(AESUtil.decrypt(k_dec1, res_i.get(j))) + ",");
